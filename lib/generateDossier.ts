// lib/generateDossier.ts  —  V2 (corrigé pour le vrai schéma Ardalos CRM)
// ============================================================================
// Génère un dossier Qualiopi/AFDAS complet (10 docx) pour une inscription.
// Fetch les données Supabase via 3 requêtes séparées (tolère FK non déclarées),
// remplit les templates avec docxtemplater, renvoie un Buffer ZIP.
//
// Corrections V2 :
//  - formations.duree (pas duree_heures)
//  - formations.objectifs (pas description)
//  - formation_sessions.formateur_principal est un TEXTE (pas un FK)
//  - .maybeSingle() partout (pas .single())
//  - Requêtes séparées au lieu d'un gros JOIN avec !inner
// ============================================================================
...
// Liste des templates à inclure dans le dossier généré
const TEMPLATES = [
  '01_CONVENTION_DE_FORMATION_PROFESSIONNELLE.docx',
  '02_PROGRAMME_DE_FORMATION.docx',
  '03_ANNEXE_1_GRILLE_EVALUATION.docx',
  '04_ANNEXE_2_FEUILLE_EMARGEMENT.docx',
  '05_ANNEXE_3_ATTESTATION_FIN_DE_FORMATION.docx',
  '06_ANNEXE_4_CERTIFICAT_DE_REALISATION.docx',
  '07_ANNEXE_5_REGLEMENT_INTERIEUR.docx',
  '08_ANNEXE_5_BIS_ATTESTATION_LECTURE_RI.docx',
  '09_ANNEXE_6_QUESTIONNAIRE_SATISFACTION.docx',
  '10_ATTESTATION_FONCTION_DIRIGEANT.docx',
]
...
export async function fetchInscriptionData(inscriptionId: string, authToken?: string | null) {
  ...
  // 4. Formation (via session.formation_id)
  ...
  // Formateur principal (texte libre dans formation_sessions)
...
export function buildVariables(data: any): Record<string, string> {
  ...
  return {
    ...
    // ==== Dirigeant (pour attestation fonction) ====
...
// ============================================================================
// Remplit un template docx avec les variables et renvoie un Buffer
// ============================================================================
...
// ============================================================================
// Génère le dossier complet : renvoie un Buffer ZIP contenant les 10 docx
// ============================================================================
export async function generateDossier(inscriptionId: string, authToken?: string | null): Promise<{ buffer: Buffer; filename: string }> {
